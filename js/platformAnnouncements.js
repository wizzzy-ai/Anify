(function () {
    ...
    fetch('/api/announcements', { cache: 'no-store' })
        ...
        .then(data => {
            ...
            card.innerHTML = `<div class="absolute inset-y-0 left-0 w-1 bg-gradient-to-b from-gold-300 via-gold-500 to-amber-600"></div><div class="p-6"><button class="announcement-popup-close absolute right-4 top-3 text-lg transition" aria-label="Dismiss">×</button><p class="text-xs font-black uppercase tracking-[0.16em] text-gold-400">${String(item.type || 'announcement').replace('_', ' ')}</p><h2 class="announcement-popup-title mt-4 pr-7 text-xl font-black leading-8">${item.title}</h2><p class="announcement-popup-message mt-3 text-[15px] leading-7">${item.message}</p>${item.actionLabel ? `<button class="mt-6 rounded-xl bg-gradient-to-r from-gold-300 to-gold-500 px-5 py-3 text-sm font-black text-[#1b1100] shadow-lg shadow-gold-500/20 transition hover:-translate-y-0.5">${item.actionLabel}</button>` : ''}</div>`;
